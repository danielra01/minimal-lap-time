...
\begin{document}
	...
	\begin{abstract}
    %Example of an abstract from other project 
	%This report studies hand-written digit classification on the \emph{sklearn} Digits dataset using multinomial logistic regression trained with gradient-based methods. We cast training as a finite-sum convex optimization problem and compare full-batch gradient descent (full-GD) against mini-batch stochastic gradient descent (SGD) in terms of objective decrease, validation/test accuracy, and wall-clock time.

	%Based on systematic hyperparameter sweeps, we observe that SGD reaches high validation accuracy in substantially less wall-clock time, while full-GD exhibits smoother, deterministic objective trajectories but incurs higher per-iteration cost. Both approaches achieve strong test performance, with SGD providing the best time-to-accuracy trade-off in our experimental setup.

	%This report documents the final project for the lecture \emph{Basics in Applied Mathematics}, offered in the winter term 2025/2026 within the master’s program \emph{Mathematics in Data and Technology} at the University of Freiburg.
	\end{abstract}
	...
	\include{optimization-problem}
	\include{numerical-solution}
	\include{results}
	\thispagestyle{empty}
	\printbibliography
\end{document}